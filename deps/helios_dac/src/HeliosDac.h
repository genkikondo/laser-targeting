...
2.	To send a frame to the DAC, first call GetStatus(). If the function
returns ready (1), then you can call WriteFrame(). The status should be polled
until it returns ready. It can and sometimes will fail to return ready on the
first try.
3.  To stop output, use Stop(). To restart output you must send a new frame as
described above. 4.	When the DAC is no longer needed, destroy the instance
(destructors will free everything and close the connection)

The DAC is double-buffered. When it receives its first frame, it starts
outputting it. When a second frame is sent to the DAC while the first frame is
being played, the second frame is stored in the DACs memory until the first
frame finishes playback, at which point the second, buffered frame will start
playing. If the DAC finished playback of a frame without having received and
buffered a second frame, it will by default loop the first frame until a new
frame is received (but the flag HELIOS_FLAG_SINGLE_MODE will make it stop
playback instead). The GetStatus() function actually checks whether or not the
buffer on the DAC is empty or full. If it is full, the DAC cannot receive a new
frame until the currently playing frame finishes, freeing up the buffer.
*/
...
// Attempted to send a new frame with HELIOS_FLAGS_DONT_BLOCK before previous
// DoFrame() completed
#define HELIOS_ERROR_DEVICE_FRAME_READY -1001
// Operation failed because SendControl() failed (if operation failed because of
// libusb_interrupt_transfer failure, the error code will be a libusb error
// instead)
...
// Errors from libusb are the libusb error code added to -5000. See libusb.h for
// libusb error codes.
...
class HeliosDac {
 public:
  ...
  // unless otherwise specified, functions return HELIOS_SUCCESS if OK, and
  // HELIOS_ERROR if something went wrong.

  // initializes drivers, opens connection to all devices.
  // Returns number of available devices.
  // NB: To re-scan for newly connected DACs after this function has once been
  // called before, you must first call CloseDevices()
  int OpenDevices();
  ...
  // ) pps: rate of output in points per second flags: (default is 0) 	Bit 0
  //(LSB) = if 1, start output immediately, instead of waiting for current frame
  //(if there is one) to finish playing 	Bit 1 = if 1, play frame only once,
  //instead of repeating until another frame is written
  //   Bit 2 = if 1, don't let WriteFrame() block execution while waiting for
  //   the transfer to finish
  //			(NB: then the function might return 1 even if it fails)
  //	Bit 3-7 = reserved
  // points: pointer to point data. See point structure declaration earlier in
  // this document numOfPoints: number of points in the frame
  int WriteFrame(unsigned int devNum, unsigned int pps, std::uint8_t flags,
                 HeliosPoint* points, unsigned int numOfPoints);

  // Gets status of DAC, 1 means DAC is ready to receive frame, 0 means it is
  // not
  ...
  // Erase the firmware of the DAC, allowing it to be updated by accessing the
  // SAM-BA bootloader
 ...
 private:
  class HeliosDacDevice  // individual dac, interal use
  {
   public:
    ...
    ~HeliosDacDevice();
    int SendFrame(unsigned int pps, std::uint8_t flags, HeliosPoint* points,
